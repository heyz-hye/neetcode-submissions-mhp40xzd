...
class Solution:
    def diameterOfBinaryTree(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node):
            if not node:
                return 0
            current_diameter=dfs(node.left)+dfs(node.right) #you compute the diameter
            self.max_diameter=max(self.max_diameter, current_diameter) #you compare diameter of all the nodes
            
            return 1+max(dfs(node.left),dfs(node.right)) #you set the height of the node
