...
elongate the actual minimual amount of time(CPU cycles needed) needed
we only care about the frequency and not the task so there is no point sorting the maxheap with the task attach as a tuple,
it mess up my popping of heaqheapop since i cannot index access a value that has only one element, it also mess my heappush method
since i need to consistently push the matching type data back. if f=heapq.heappop() f=1 i cant do heapq.heappop()[0]
