...
There is two condition that break the while loop, the one condition is that queue finish that is no element to popped and explore in the queue but there is
still fresh fruit remaining that is unreachable in that case we return -1 or the other case where the fresh fruit is 0 but there is still something inside queue which
